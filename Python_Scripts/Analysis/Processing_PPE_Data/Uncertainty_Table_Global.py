...
# This script creates the table needed for Table of which just shows the GLOBAL std for each variable in ERF.
...
Predicted_Control, _ = gp_model_.predict(norm_param.values)
...
for col,i in zip(para.columns,range(0,len(para.columns))):
    ...
    results.append({
        ...
        f"5%_{variable}": np.percentile(gpmeans,5).data,
        f"95%_{variable}": np.percentile(gpmeans,95).data,
        f"range_{variable}": (np.percentile(gpmeans,5) - np.percentile(gpmeans,95)).data,
        # 'ERF_Contribution': ERF_Contribution.data,
        # 'ERF_Contribution_PERC': ERF_Contribution_PERC.data,
        f"%range_{variable}": ((np.percentile(gpmeans,95) - np.percentile(gpmeans,5)))*100,

#        'CNN_std': nnmeans,
#        'Diff': difference
    })
 #   results_ = pd.DataFrame(results)
    #results_.to_csv(f"/home/ybhatti/prjs1076/Processed_Data/Analysed_Data/Raw/Table_of_Uncertainty_{i}.csv", index=False)

# Convert the accumulated results into a DataFrame after the loop
results_df = pd.DataFrame(results)
results_df[f"Uncertainty_{variable}"] = (results_df['std'] / results_df['std'].sum()) * 100



# Sort the DataFrame by the 'Diff' column
results_df.to_csv(f"/home/ybhatti/prjs1076/Processed_Data/Analysed_Data/CDNC_40/Table_of_Global_Uncertainty_{n}_{variable}.csv", index=False)



variable = 'SSA_PD'
#ppe_var=copy.deepcopy(AOD_PD.mean('month'))
